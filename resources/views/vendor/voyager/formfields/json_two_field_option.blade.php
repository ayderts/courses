...
<div class="custom-parameters">
        ...
        @foreach($old_parameters as $parameter)
            <div class="form-group row" row-id="{{$loop->index}}">
                <div class="col-xs-3" style="margin-bottom:0;">
                    <input type="text" class="form-control" name="{{ $row->field }}[{{$loop->index}}][field_one]"
                           value="{{ $parameter['field_one'] }}" id="field_one"/>
                </div>

                <div class="col-xs-3" style="margin-bottom:0;">
                    <input type="text" class="form-control" name="{{ $row->field }}[{{$loop->index}}][field_two]"
                           value="{{ $parameter['field_two'] }}" id="field_two"/>
                </div>
    ...
    <div class="form-group row" row-id="{{ $end_id }}">
        <div class="col-xs-3" style="margin-bottom:0;">
            <input type="text" class="form-control" name="{{ $row->field }}[{{ $end_id }}][field_one]" value=""
                   id="field_one"/>
        </div>

        <div class="col-xs-3" style="margin-bottom:0;">
            <input type="text" class="form-control" name="{{ $row->field }}[{{ $end_id }}][field_two]" value=""
                   id="field_two"/>
        </div>

        <div class="col-xs-1" style="margin-bottom:0;">
            <button type="button" class="btn btn-success btn-xs" style="margin-top:0px;"><i class="voyager-plus"></i>
            </button>
        </div>
    </div>

    <input type="hidden" name="key_value_json" value="{{$row->field}}"/>
</div>
...
<script>
    ...
    function addRow() {
        ...
        new_row.querySelector("#field_one").setAttribute('name', editNameCount(new_row.querySelector("#field_one")));
        new_row.querySelector("#field_one").value = '';
        new_row.querySelector("#field_two").setAttribute('name', editNameCount(new_row.querySelector("#field_two")));
        new_row.querySelector("#field_two").value = '';

...
@section('page_header')
    <div class="container-fluid">
        ...
        @if($treeMode)
            <a href="{{ route('voyager.'.$dataType->slug.'.index', ['type' => 'tree']) }}" class="btn btn-warning">
                <i class="voyager-tree"></i> <span>{{ __('voyager::generic.tree_view') }}</span>
            </a>
        @endif
                                    ...
                                    <tr data-record-id="{{$data->getKey()}}"
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td class="@if(isset($row->details->browse_align)){{ $row->details->browse_align }}@endif"
                                                ...
                                                @elseif ($row->field == 'position')
                                                    @php
                                                        $order_direction = $dataType->details->order_direction ?? 'DESC';
                                                    @endphp
                                                    <a href="{{route('order_row', ['position' => $data->{$row->field}, 'table' => $dataType->name, 'sort' => $order_direction, 'direction' => 'FIRST'])}}">
                                                        <img
                                                            src="{{URL::asset('/voyager-assets/images/position/first.png')}}"
                                                            style="width:26px"/>
                                                    </a>
                                                    <a href="{{route('order_row', ['position' => $data->{$row->field}, 'table' => $dataType->name, 'sort' => $order_direction, 'direction' => 'UP'])}}">
                                                        <img
                                                            src="{{URL::asset('/voyager-assets/images/position/up.png')}}"
                                                            style="width:31px"/>
                                                    </a>
                                                    <a href="{{route('order_row', ['position' => $data->{$row->field}, 'table' => $dataType->name, 'sort' => $order_direction, 'direction' => 'DOWN'])}}">
                                                        <img
                                                            src="{{URL::asset('/voyager-assets/images/position/down.png')}}"
                                                            style="width:31px"/>
                                                    </a>
                                                    <a href="{{route('order_row', ['position' => $data->{$row->field}, 'table' => $dataType->name, 'sort' => $order_direction, 'direction' => 'LAST'])}}">
                                                        <img
                                                            src="{{URL::asset('/voyager-assets/images/position/last.png')}}"
                                                            style="width:27px"/>
                                                    </a>
